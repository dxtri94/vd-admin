    ...
    init: function (options) {
      ...
      return this.each(function () {
        ...
        var prepareData = function () {
          ...
          if (settings.minDate) {
            if (settings.format.indexOf("L") >= 0) {
              minTime = moment(settings.minDate, "YYYY.MM.DD").locale(settings.lang);
            } else {
              minTime = moment(settings.minDate, settings.format).locale(settings.lang);
            }
          }
          ...
          if (settings.minDate) {
            if (settings.format.indexOf("L") >= 0) {
              maxTime = moment(settings.maxDate, "YYYY.MM.DD").locale(settings.lang);
            } else {
              maxTime = moment(settings.maxDate, settings.format).locale(settings.lang);
            }
          }
        ...
        var prepareCalendar = function () {
          ...
          if (settings.sundayFirst) {
            ...
            for (i = 1; i <= monthLastDay; i++) {
              // current day - start cell
              if (moment(timeNowLocal).date(i).format("D.M.YYYY") === timeNow.format("D.M.YYYY")) {
                html += '<td class="ic__day ic__day_state_current">';
              } else if (timeSelected && moment(timeNowLocal).date(i).format("D.M.YYYY") === timeSelected.format("D.M.YYYY")) {
                html += '<td class="ic__day ic__day_state_selected">';
              } else {
                html += '<td class="ic__day">';
              }

              // number button
              html += '<button class="btn-day">' + i + '</button>';
            ...
            for (i = 1; i <= monthLastDay; i++) {
              ...
              // custom class name
              var className = 'ic__day';

              var date = moment(new Date(timeNowLocal.format("Y"), timeNowLocal.format("M") - 1, i));

              if (minTime && date.isBefore(minTime)) {
                className += '-min'
              }

              if (maxTime && date.isAfter(maxTime)) {
                className += '-max';
              }
              ...
              // current day
              if (moment(timeNowLocal).date(i).format("D.M.YYYY") === timeNow.format("D.M.YYYY")) {
                html += '<td class="' + className + ' ic__day_state_current">';
              } else if (timeSelected && moment(timeNowLocal).date(i).format("D.M.YYYY") === timeSelected.format("D.M.YYYY")) {
                html += '<td class="' + className + ' ic__day_state_selected">';
              } else {
            ...
            for (i = weekLastDay - 1; i < 6; i++) {
              html += '<td class="ic__day-empty"><button>&nbsp;</button></td>';
            }
            html += '</tr></table>';
          }
    ...
    init: function (options) {
      ...
      return this.each(function () {
        ...
        settings.onClick = function (date) {
          $input.val(date).triggerHandler('input');
          selectedDate = date;
          if (typeof (settings.callback) === 'function') {
            settings.callback(date);
          }
          closePopups();
        };
        ...
        var openPopup = function () {
          ...
          wp = parseInt($popup.outerWidth(true));
